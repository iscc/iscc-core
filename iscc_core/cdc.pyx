# -*- coding: utf-8 -*-
"""Cython implementation of Content-Defined Chunking."""
import io
...
cdef uint32_t MINIMUM_MIN = 64
cdef uint32_t MINIMUM_MAX = 67_108_864
cdef uint32_t AVERAGE_MIN = 256
cdef uint32_t AVERAGE_MAX = 268_435_456
cdef uint32_t MAXIMUM_MIN = 1024
cdef uint32_t MAXIMUM_MAX = 1_073_741_824
...
def alg_cdc_params(avg_size):
    # type: (int) -> tuple[int, int, int, int, int]
    """
    ...
    return min_size, max_size, cs, mask_s, mask_l
...
def alg_cdc_chunks(data, utf32, avg_chunk_size=ic.core_opts.data_avg_chunk_size):
    ...
    max_size = avg_chunk_size * 8
    bits = logarithm2(avg_chunk_size)
    mask_s = mask(bits + 1)
    mask_l = mask(bits - 1)
    cs = center_size(avg_chunk_size, min_size, len(buffer))

    buffer = memoryview(buffer)
    while buffer:
        if len(buffer) <= max_size:
            new_data = stream.read(ic.core_opts.io_read_size)
            if new_data:
                buffer = memoryview(bytes(buffer) + new_data)
        
        cut_point = cdc_offset(buffer, min_size, max_size, cs, mask_s, mask_l)
