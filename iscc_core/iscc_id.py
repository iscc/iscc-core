...
def gen_iscc_id_v0(chain_id, iscc_code, uc=0):
    ...
    iscc_id_code = codec.encode_component(
        mtype=codec.MT.ID,
        stype=chain_id,
        version=codec.VS.V0,
        length=iscc_id_len,
...
def soft_hash_iscc_id_v0(iscc_code_digest, uc=0):
    # type: (bytes, int) -> bytes
    """Calculate ISCC-ID hash digest from ISCC-CODE digest with algorithm v0"""
    components = codec.decompose(iscc_code_digest)
    digests = [
