...
from iscc_core.base import Data, CDC_AVG_CHUNK_SIZE, CDC_READ_SIZE
...
def data_chunks(data, utf32=False, avg_chunk_size=CDC_AVG_CHUNK_SIZE):
    ...
    buffer = stream.read(CDC_READ_SIZE)
    if not buffer:
        yield b""
    ...
    while buffer:
        if len(buffer) <= ma:
            buffer = memoryview(bytes(buffer) + stream.read(CDC_READ_SIZE))
