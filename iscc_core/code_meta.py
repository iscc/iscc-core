# -*- coding: utf-8 -*-
"""
The Meta-Code is the first component of a fully qualified ISCC. It is calculated as a
...
The metadata we supply to the algorithm is called *seed metadata*.
...
from iscc_core.codec import MT, ST_MC, VS, encode_component
...
def gen_meta_code(title, extra=None, bits=opts.meta_bits):
    ...
    information. If not available they shall try to extract a title form the the digital
    asset itself. If extraction fails, the application shall resort to the filename
...
def gen_meta_code_v0(title, extra=None, bits=opts.meta_bits):
    ...
    # 2. Normalize extra and set Meta-Code SubType
    if extra in (None, ""):
        extra = None
        stype = ST_MC.TEXT
...
def hash_meta_v0(title, extra=None):
    ...
    """Calculate simmilarity preserving 256-bit hash digest for provided metadata.
    ...
    !!! note
        If the `extra` field is supplied we create separate hashes for title and extra
        and interleave them in 32-bit chunks. The processign algorithms and the subtype
        of the generated hash digest depends on the type the `extra` input.
        If the input is a **text string** (str) the processing unit is a character
        (possibly multibyte) and the resulting hash type is **ST_MC.TEXT (0)**. If the
        input is **bytes** the processing is done bytewise and the resulting hash type
        is **ST_MC.BYTES (1)**.
    ...
    :return: 256-bit simhash digest for Meta-Code of SubType ST_MC.TEXT or ST.MC_BYTES
    :rtype: bytes
    """
    title = title.lower()
