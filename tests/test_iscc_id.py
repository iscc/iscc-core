...
def test_gen_iscc_id_v0_single_component():
    ...
    assert iscc_core.explain(iscc_id["iscc"]) == "ID-PRIVATE-V0-64-20250db96e0d4a17"
...
def test_gen_iscc_id_v0_single_component_uc():
    ...
    assert iscc_core.explain(iscc_id["iscc"]) == "ID-PRIVATE-V0-72-20250db96e0d4a17-1"
...
def test_gen_iscc_id_v0_single_component_uc_2byte():
    ...
    assert iscc_core.explain(iscc_id["iscc"]) == "ID-PRIVATE-V0-80-20250db96e0d4a17-257"


def test_gen_iscc_id_v0_multiple_components():
    mc = iscc_core.gen_meta_code_v0("Some Title")["iscc"]
    tc = iscc_core.gen_text_code_v0("Hello World")["iscc"]
    code = iscc_core.clean(mc) + iscc_core.clean(tc)
    iscc_id = iscc_core.gen_iscc_id(1, code)
    assert iscc_id["iscc"] == "ISCC:MEACANI57VXZ67R7"
    assert iscc_core.explain(iscc_id["iscc"]) == "ID-BITCOIN-V0-64-20351dfd6f9f7e3f"
...
def test_gen_iscc_id_v0_instance_only():
    ...
    assert iscc_core.explain(iscc_id["iscc"]) == "ID-PRIVATE-V0-64-40d74981efa70a0c"
...
def test_gen_iscc_id_v0_data_instance():
    ...
    assert iscc_core.explain(iscc_id["iscc"]) == "ID-PRIVATE-V0-64-30fec3fcc5db9170"
