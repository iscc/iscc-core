...
def test_hash_video_v0_features():
    assert code_content_video.hash_video_v0([tuple([0] * 380)]) == b"\x00" * 32


def test_hash_video_v0_range():
    frame_vectors = [tuple(range(380))]
    assert (
        code_content_video.hash_video_v0(frame_vectors).hex()
...
def test_code_video_v0_features():
    assert code_content_video.code_video_v0([tuple([0] * 380)]) == "EMAQAAAAAAAAAAAA"


def test_code_video_v0_range_128():
    frame_vectors = [tuple(range(380))]
    assert (
        code_content_video.code_video_v0(frame_vectors, bits=128)
...
def test_code_video_v0_multiple_framevectors_256():
    ...
    frame_vectors = [fa, fb]
    assert (
        code_content_video.code_video_v0(frame_vectors, bits=256)
